...
export const CurrencyConverter = () => {
  ...
  if (isBaseSource) {
    ...
    targetVal = !isNaN(num)
      ? safeFormat(convertFiat(num, baseCurrency, targetCurrency))
      : "";
  } else {
    targetVal = localAmount;
    const num = parseFloat(localAmount);
    baseVal = !isNaN(num)
      ? safeFormat(convertFiat(num, targetCurrency, baseCurrency))
      : "";
  }
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 md:p-10 border border-slate-100 dark:border-slate-700 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-end">
        ...
        <div className="flex justify-center md:pb-3">
          <button
            onClick={handleSwap}
            className="p-3 rounded-full bg-blue-50 dark:bg-slate-700 text-blue-600 dark:text-blue-400 hover:bg-blue-100 hover:scale-110 transition-all shadow-sm border border-blue-100"
          >
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center text-sm text-slate-500">
        <p>
          1 {baseCurrency} ={" "}
          {convertFiat(1, baseCurrency, targetCurrency).toFixed(4)}{" "}
          {targetCurrency}
        </p>
        <p className="flex items-center">
          <Activity className="h-4 w-4 mr-1" /> Updated{" "}
