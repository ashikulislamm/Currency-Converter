    ...
    <nav className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
        <a href="/" className="flex items-center cursor-pointer">
          <img
            src={Logo.src}
            alt="GlobalXchange Logo"
            className="h-8 w-8 text-blue-600 mr-2"
          />
          <span className="font-bold text-xl">
            Global<span className="text-blue-600">X</span>change
          </span>
        </a>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-2">
          <a
            ...
            className="block w-full text-left p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded"
          >
            Converter
          </a>

          <div className="border-t border-slate-200 dark:border-slate-700 pt-2 mt-2">
            <p className="text-xs text-slate-500 px-2 mb-2 font-semibold">
              CURRENCIES
            </p>
            {currencies.map((c) => (
              <a
