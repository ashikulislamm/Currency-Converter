...
export const SUPPORTED_CURRENCIES: CurrencyCode[] = ['USD', 'EUR', 'BDT', 'GBP', 'JPY', 'CAD', 'AUD', 'CNY'];
...
export const CurrencyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [baseCurrency, setBaseCurrency] = useLocalStorage<CurrencyCode>('baseCurrency', 'USD');
  const [targetCurrency, setTargetCurrency] = useLocalStorage<CurrencyCode>('targetCurrency', 'BDT');
  ...
  // Fetch Crypto
  useEffect(() => {
    const streams = SUPPORTED_CRYPTO.map(c => `${c.toLowerCase()}usdt@trade`).join('/');
  ...
  const convertFiat = useCallback((amount: number, from: string, to: string) => {
